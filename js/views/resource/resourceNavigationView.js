...
define(
    ...
    function(Common, SubServiceMenuView, CloudCredentials, DataCenters,
             cloudCredential, resourcesTemplate, breadcrumbTemplate) {
        ...
        var ResourcesView = Backbone.View.extend({
          ...
          render: function() {
            if ($("#cloud_coverflow").children().length > 0) {
              ...
              if (this.resourceApp) {
                ...
                policy = Common.account.group_policies;
                if (policy.length > 0) {
                ...
                if (this.cloudProvider) {
                  if ($("#" + this.cloudProvider).length) {
                    ...
                    Common.router.navigate("#resources/" + this.cloudProvider, {
                      trigger: false
                    });
                  } else {
                    Common.router.navigate("#resources/" + firstCloudProvider, {
                      trigger: false
                    });
                    this.cloudSelection(firstCloudProvider);
                  }
                } else if (Common.cache('selected_cloud') !== undefined) {
                  Common.router.navigate("#resources/" + Common.cache('selected_cloud'), {
                    trigger: false
                  });
                  this.cloudSelection(Common.cache('selected_cloud'));
                } else {
                  Common.router.navigate("#resources/" + firstCloudProvider, {
                    trigger: false
                  });
          ...
          enableCloud: function(cloudPolicies, provider) {
            ...
            var check = false;
            //Admin check
            if (Common.account.permissions.length > 0) {
              check = false;
            } else {
          ...
          addCloud: function(cloudCredential) {
            ...
            var cloudPolicies = Common.account.group_policies;
            if (cloudProvider) {
              cloudProvider = cloudProvider.toLowerCase();
              var found = false;
              var selected_cloud = Common.cache('selected_cloud');
          ...
          cloudChange: function(event) {
            if (!event.isTrigger) {
              ...
              Common.router.navigate("#resources/" + event.target.id, {
                trigger: false
              });
          ...
          cloudSelectionIterator: function(collection, formElement) {
            ...
            var governance = Common.account.group_policies;
          ...
          cloudSelection: function(cloudProvider) {
            ...
            Common.cache('selected_cloud', this.cloudProvider);
            var resourceNav = this;
            ...
            $("#cloud_nav").html(this.crumbTemplate({
              pathElt: this.cloudDefinitions[this.cloudProvider].name
            }));
            this.type = undefined;
          ...
          credentialChange: function(event) {
            ...
            Common.cache('selected_cred_' + this.cloudProvider, this.selectedCredential);
          ...
          refreshCredentials: function() {
            ...
            if (Common.cache('selected_cred_' + this.cloudProvider) !== undefined) {
              $("#credential_select").val(Common.cache('selected_cred_' + this.cloudProvider));
            ...
            if (this.cloudProvider) {
              ...
              if (this.type === "admin") {
                ...
                if (serviceObject && serviceObject.useSubServiceFolderNames) {
                  folder = _.find(serviceObject.subServices, {
        ...
        Common.router.on('route:resources', function(cloud, region, type, subtype, id) {
          ...
          if (account.cloud_credentials.length <= 0) {
            Common.router.navigate("#account/management/cloud-credentials_list", {
              trigger: true
            });
