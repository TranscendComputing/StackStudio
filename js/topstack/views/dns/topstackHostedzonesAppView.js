...
define([
        ...
        'text!templates/topstack/dns/topstackHostedZoneAppTemplate.html',
        '/js/topstack/models/dns/topstackHostedZone.js',
        '/js/topstack/collections/dns/topstackHostedZones.js',
        '/js/topstack/models/dns/topstackRecordSet.js',
        '/js/topstack/collections/dns/topstackRecordSets.js',
        'views/resource/resourceRowView',
        'icanhaz',
        'common',
        'jquery.dataTables'
], function( $, _, Backbone, ResourceAppView, topstackHostedZoneAppTemplate, HostedZone, HostedZones, RecordSet, RecordSets, ResourceRowView, ich, Common ) {
    'use strict';

    var TopStackHostedZoneAppView = ResourceAppView.extend({
        
        template: _.template(topstackHostedZoneAppTemplate),
        ...
        refreshRecordSetsTab: function() {
            ...
            this.$recordSetTable = $('#record_set_table').dataTable({
                "bJQueryUI": true,
                "sDom": "t",
                "iDisplayLength": -1
            });
        ...
        createRecordSet: function () {
            ...
            var RecordSetCreateView = this.RecordSetCreateView;
            var recordSetCreate = new RecordSetCreateView({cred_id: this.credentialId, hosted_zone: hostedZone});
            recordSetCreate.render();
        },
        ...
        performRecordSetAction: function(event) {
            if(this.selectedRecord) {
                ...
                case "Edit Record Set":
                    var RecordSetCreateView = this.RecordSetCreateView;
                    var recordSetCreate = new RecordSetCreateView({cred_id: this.credentialId, hosted_zone: hostedZone, record_set:this.selectedRecord});
                    recordSetCreate.render();
                    break;
    ...
    return TopStackHostedZoneAppView;
});
