package com.momentumsi.c9.converters
{
	...
	public class XML2JSON {
		...
		public static function parse(node:*):Object {
				...
				if(childNode.children().length() == 1 && childNode.children()[0].name() == null) {
					if(childNode.attributes().length() > 0) {
						...
						for(var j:int=0; j<numOfAttributes; j++) {
							value[String(childNode.attributes()[j].name()).toString()] = childNode.attributes()[j];
						}
					} else {
			...
			for(i=0; i<numOfAttributes; i++) {
				obj[String(node.attributes()[i].name()).toString()] = node.attributes()[i];
			}
			if(numOfChilds == 0) {
