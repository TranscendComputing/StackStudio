        ...
        try {
            ...
            case SMSG_EXITSTATUS:
                SSHPduOutputStream exitPdu =
                    new SSHPduOutputStream(CMSG_EXIT_CONFIRMATION, sndCipher,
                                           sndComp, secureRandom());
                int status = inPdu.readInt();
                if(console != null) {
                    if(status != 0)
                        console.serverDisconnect(
                            sshAsClient().getServerAddr().getHostName() +
                            " disconnected: " + status);
                    else
                        console.serverDisconnect(
                            "Connection to " +
            ...
            case MSG_CHANNEL_OPEN_CONFIRMATION:
                ...
                if(tunnel != null) {
                    if(!tunnel.setRemoteChannelId(inPdu.readInt()))
                        throw new Exception(
                            "Open confirmation on allready opened channel!");
                    tunnel.start();
                } else
