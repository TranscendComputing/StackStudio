...
public class Switchboard implements Runnable {
     ...
     *              is established
     *
     * @return the new network conenction object. Note that this
    ...
    public final void debug2(String cls, String meth, String msg) { 
        try {
            if (_log != null)
                _log.debug2(cls, meth, msg);
        } catch (Throwable t) {
            // t.printStackTrace();
        }
    }

    private void dbg(String meth, String msg) { 
        try {
            if (_log != null)
                _log.debug2("Switchboard", meth,
                            "t@" + Thread.currentThread().getId() + " " + msg);
        } catch (Throwable t) {
        }
    }    
    ...
    protected void read(AbstractSelectableChannel channel, ByteBuffer buf,
        ...
        try {
            ...
            if (key == null) {
                ...
                dbg("read", "new key for channel=" + channel + ", key="+key);
            } else if (!key.isValid()) {
                dbg("read", "channel=" + channel + ", invalid key=" + key);
            } else if ((key.interestOps() & OP_READ) == 0) {
                dbg("read", "channel=" + channel + ", key exists, key=" + key);                
                ...
                dbg("read", "channel=" + channel + ", already reading, key=" + key);
            }
        } finally {
            _channelslock.unlock();
        }
    ...
    protected void write(AbstractSelectableChannel channel, ByteBuffer buf,
        ...
        try {
            ...
            if (key == null) {
                ...
                dbg("write", "new key for channel: " + channel + ", key="+key);
            } else if ((key.interestOps() & OP_WRITE) == 0) {
                dbg("write", "preparing write, key=" + key);
                ChannelData data = _channels.get(key);
                data.prepareWrite(buf, lock);
                key.interestOps(key.interestOps() | OP_WRITE);
            } else {
                dbg("write", "adding data");
                ChannelData data = _channels.get(key);
                if (data._writeLock != lock)
                    dbg("write", "data._writeLock != lock");
                data.prepareWrite(buf, lock);
            }
        } finally {
            _channelslock.unlock();
            ...
            try {
                ...
                synchronized (_timers) {
                    ...
                    while (iter.hasNext()) {
                        ...
                        if (now >= t._next) {
                            try {
                                dbg("run", "calling timer trig: " + t._callback);
                    ...
                    try {
                        ...
                        while (iter.hasNext()) {
                            ...
                            if (data._timeoutWhen != 0 && data._timeoutWhen < now) {
                                data._timeoutWhen = 0;
                                try {
                                    dbg("run", "calling connected callback timeout: " + data._callback);
    ...
    private void handleKey(SelectionKey key) throws CancelledKeyException {
        ...
        if (key.isConnectable() && (key.interestOps() & OP_CONNECT) != 0) {
            try {
                ...
                try {
                    dbg("handleKey",
                        "calling connected callback: " + data._callback);
                    data._callback.connected(false);
                } catch (Throwable t) {}
            } catch (Exception e) {
                try {
                    dbg("handleKey",
                        "calling connFail callback: " + data._callback);
        ...
        if (key.isReadable() && (key.interestOps() & OP_READ) != 0) {
            ...
            try {
                ...
                dbg("handleKey", "read, channel=" + data._channel + ", n=" + n);
                dbg("handleKey",
                    "read, channel=" + data._channel + ", buf=" + buf);
            } catch (IOException ioe) {
                dbg("handleKey",
                    "read failed, channel=" + data._channel + ", " + ioe);
                if (_log != null)
                    _log.debug(ioe);
                e = ioe;
            }
            ...
            if (n < 1 || e != null) {
                dbg("handleKey", "read failed, channel=" + data._channel);
                key.interestOps(key.interestOps() & ~OP_READ);
                data._readLock.clearReadWaiting();
                try {
                    dbg("handleKey", "calling read failed: " + data._callback);
                    data._callback.readFailed(e);
                } catch (Throwable t) {
                    // t.printStackTrace();
                }
            } else if (buf.remaining() == 0 || data._shortReadOk) {
                dbg("handleKey", "read done, channel=" + data._channel);
                key.interestOps(key.interestOps() & ~OP_READ);
                data._readLock.clearReadWaiting();
                try {
                    dbg("handleKey", "calling read completed "
        ...
        if (key.isWritable() && (key.interestOps() & OP_WRITE) != 0) {
            dbg("handleKey", "write, channel=" + data._channel);

            while (data._writeBufs.size() > 0) {
                ByteBuffer buf=data._writeBufs.getFirst();
                try {
                    dbg("handleKey",
                        "write, channel=" + data._channel + ", buf=" + buf);
                    int n = ((WritableByteChannel)data._channel).write(buf);
                    dbg("handleKey",
                        "write, channel=" + data._channel + ", n=" + n);
                } catch (IOException e) {
                    dbg("handleKey", "write failed, channel=" + data._channel);
                    if (_log != null)
                        _log.debug(e);
                    ...
                    try {
                        dbg("handleKey", 
                            "calling write failed: " + data._callback);
