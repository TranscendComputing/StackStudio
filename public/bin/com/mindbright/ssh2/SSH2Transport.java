...
public class SSH2Transport implements NIOCallback, NQueueCallback {
	...
	protected int                discard = 0;
        ...
        public TranceiverContext(NonBlockingInput in) {
            this.in = in;
        }
	...
	private void cipherDiscard() {
        ...
        public void init(byte[] cKey, byte[] iv, byte[] mKey, int compLevel,
		    ...
		    cipherDiscard();
                ...
                throw new SSH2FatalException("Invalid algorithm parameter in TranceiverContext.init");                
    ...
    private NQueue txInternalQueue;
    ...
    public SSH2Transport(NetworkConnection tpSocket, SSH2Preferences prefs,
        ...
        try {
            ...
            this.rxContext =
                SSH2TransportPDU.createTranceiverContext("none", 0,
							 "none", 0,
                                                         "none", tpIn);
            this.txContext =
                SSH2TransportPDU.createTranceiverContext("none", 0,
    ...
    public boolean waitForKEXComplete() throws SSH2Exception {
        ...
        synchronized(keyExchangeMonitor) {
            if (keyExchangeInProgress || setupInProgress) {
                try {
                    long timeout = 0;
                    if (setupInProgress) {
    ...
    public void handleQueue(Object obj) {
        ...
        try {
            ...
            String msg = "I/O error1: " + e.getMessage();
            if(isTxUp) {
                tpLog.error("SSH2Transport", "handleQueue", msg);
            }
            disconnectInternal(SSH2.DISCONNECT_CONNECTION_LOST, msg,
    ...
    throws SSH2Exception {
		...
		cDiscard = 1536;
            ...
            TranceiverContext ctx =
                SSH2TransportPDU.createTranceiverContext
    ...
    public void completed(ByteBuffer buf) {
        try {
            ...
            } while(buf != null && tpIn.read(buf, this, true, false));
        } catch (IOException e) {
            if(isRxUp) {
                String msg = "I/O error2: " + e.getMessage();
                disconnectInternal(SSH2.DISCONNECT_CONNECTION_LOST, msg,
                                   null, false);
            }
        } catch (Throwable t) {
    ...
    public void readFailed(Exception e) {
        String msg = "I/O error3: " + ((e != null) ? e.getMessage() : "unknown error");
        disconnectInternal(SSH2.DISCONNECT_CONNECTION_LOST, msg, null, false);
    }

    public void writeFailed() {
        String msg = "I/O error4: write failed";
        disconnectInternal(SSH2.DISCONNECT_CONNECTION_LOST, msg, null, false);
    }
    ...
    public void connected(boolean timeout) {
        ...
        if (weAreAServer) {
            ...
            try {
                ...
                ByteBuffer buf = tpIn.createBuffer(new byte[1]);
                tpIn.read(buf, this);
            } catch (IOException e) {
    ...
    private ByteBuffer handleReadData(ByteBuffer buf) {
        ...
        case STATE_READ_IDSTRING:
            ...
            txInternalQueue = new NQueue(this);
            ...
            return pduIn.initReceive(rxSeqNum++, rxContext);

        case STATE_READ_PDU:
            try {
                ByteBuffer more = pduIn.processData(buf);
                if (more != null) {
                    return more;
                }
