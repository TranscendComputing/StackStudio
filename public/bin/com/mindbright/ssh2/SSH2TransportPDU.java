...
public class SSH2TransportPDU extends SSH2DataBuffer {
	...
	createTranceiverContext(String cipherName, int cipherDiscard,
				String macName, int macLength,
				String compName, NonBlockingInput in)
        throws Exception {
        return factoryInstance.createTranceiverContextImpl
    ...
    private int bs;
    private int macSize;
    private ByteBuffer netBuf;
    private int state;
    private SSH2Transport.TranceiverContext context;
    ...
    /**
     * Prepare this PDU for receiving data. It will return a
     * ByteBuffer which the caller should fill and return via the
     * processData method.
     *
     * @param seqNum Sequence number of packet.
     * @param context Tranceiver context.
     *
     * @return a buffer which should be fileld with data from the network
     */
    public ByteBuffer initReceive(int seqNum,
        ...
        netBuf = context.in.createBuffer(data);
        netBuf.position(wPos);
        prepareForRead(bs);
        return netBuf;
    }
    ...
    private void prepareForRead(int n) {
        ...
        try {
            if((data.length - wPos) < n) {
                ...
                netBuf = context.in.createBuffer(data, wPos);
            }
     ...
     *
     * @return a ByteBuffer if more data is needed to create the
     *         entire packet. Returns null when the entire packet has
     *         been read.
     */
    public ByteBuffer processData(ByteBuffer buf) 
	...
	createTranceiverContextImpl(String cipherName, int cipherDiscard,
				    String macName, int macLength,
				    String compName, NonBlockingInput in)
    throws Exception {
        SSH2Transport.TranceiverContext ctx =
            new SSH2Transport.TranceiverContext(in);
